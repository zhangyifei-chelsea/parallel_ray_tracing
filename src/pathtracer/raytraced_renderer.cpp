...
enum accer_type {BVH, KDTREE, GRID};
...
void RaytracedRenderer::start_raytracing() {
  ...
  if (grids != NULL) {
      grids->total_isects = 0; grids->total_rays = 0;
  }
  if (kdtree != NULL) {
      kdtree->total_isects = 0; kdtree->total_rays = 0;
  }
...
void RaytracedRenderer::build_accel(int type) {
  ...
  // build BVH //
  fprintf(stdout, "[PathTracer] Building acceleration structure from %lu primitives... ", primitives.size());
  fflush(stdout);
  timer.start();
  size_t primitive_size = primitives.size();
  if (type == BVH) {
      bvh = new BVHAccel(primitives);
      timer.stop();
      fprintf(stdout, "Done! Building BVH in (%.4f sec)\n", timer.duration());
      // initial visualization //
      selectionHistory.push(bvh->get_root());
  }
  else if (type == GRID) {
      grids = new GridAccel(primitives, primitive_size * 27);
      timer.stop();
      fprintf(stdout, "Done! Building grids in (%.4f sec)\n", timer.duration());
  }
  else if (type == KDTREE) {
      kdtree = new KDTreeAccel(primitives);
      timer.stop();
      fprintf(stdout, "Done! Building KDTree in (%.4f sec)\n", timer.duration());
  }
}
