...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
  double camera_x_size = 2 * tan(this->hFov / 180 * M_PI / 2); // tan takes in an angle in radius
  double camera_y_size = 2 * tan(this->vFov / 180 * M_PI / 2);
  double converted_x = - 0.5 * camera_x_size + camera_x_size * x;
  double converted_y = - 0.5 * camera_y_size + camera_y_size * y;
  Vector3D camera_space_ray_vector(converted_x, converted_y, -1);
  Vector3D world_space_ray_vector = c2w * camera_space_ray_vector;
  return Ray(pos, world_space_ray_vector.unit()); // return a unit vector
