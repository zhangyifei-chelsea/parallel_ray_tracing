 ...
 */
        struct KDNode {
            ...
            KDNode(BBox bb): bb(bb), l(NULL), r(NULL) { }
            ...
            KDNode* l;     ///< left child node
            KDNode* r;     ///< right child node
 ...
 */
        ...
        public:
            ...
            bool has_intersection(const Ray& r, KDNode *node) const;
            ...
            bool intersect(const Ray& r, Intersection* i, KDNode *node) const;
             ...
             */
            KDNode* get_root() const { return root; }

            /**
             * Draw the BVH with OpenGL - used in visualizer
             */
            void draw(const Color& c, float alpha) const { }
            void draw(KDNode *node, const Color& c, float alpha) const;

            /**
             * Draw the BVH outline with OpenGL - used in visualizer
             */
            void drawOutline(const Color& c, float alpha) const { }
            void drawOutline(KDNode *node, const Color& c, float alpha) const;

            mutable unsigned long long total_rays, total_isects;

        private:
            std::vector<Primitive*> primitives;
            KDNode* root; ///< root node of the BVH
            KDNode *construct_bvh(std::vector<Primitive*>::iterator start, std::vector<Primitive*>::iterator end, size_t max_leaf_size);
            KDNode* construct_kd(std::vector<Primitive*>::iterator start, std::vector<Primitive*>::iterator end, size_t max_leaf_size, size_t deep, BBox bbfa, int* fnum);
        };
